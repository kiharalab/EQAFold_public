...
def premask_aa(all_atom_pred_pos, all_atom_positions, all_atom_mask):
    '''Pre-applies the mask for all-atom LDDT prediction'''
    all_atom_pred_pos = rearrange(all_atom_pred_pos, "B L A C -> B (L A) C") #Batch residueLength Atom Coord
    all_atom_positions = rearrange(all_atom_positions, "B L A C -> B (L A) C")
    all_atom_mask = rearrange(all_atom_mask, "B L A -> B (L A)")
    
    max_aa_len = int(all_atom_mask.sum(axis=1).max())
    batch_size = int(all_atom_mask.shape[0])
    pre_masked_pred = torch.zeros(batch_size, max_aa_len, 3)
    pre_masked_true = torch.zeros(batch_size, max_aa_len, 3)
    pre_masked_mask = torch.ones(batch_size, max_aa_len,1)

    for batch, (pos_true_batch, pos_pred_batch, mask_batch) in enumerate(zip(all_atom_positions, all_atom_pred_pos, all_atom_mask)):

        premasked_true_batch = pos_true_batch[mask_batch.bool()]
        pre_masked_true[batch,:premasked_true_batch.shape[0], :] = premasked_true_batch
        
        premasked_pred_batch = pos_pred_batch[mask_batch.bool()]
        pre_masked_pred[batch,:premasked_pred_batch.shape[0], :] = premasked_pred_batch

    return pre_masked_pred, pre_masked_true, pre_masked_mask


def aggregate_to_residue_level_lddt(all_atom_mask_orig, lddt_data):

    batch, length = all_atom_mask_orig.shape[0:2]
    residue_averaged_lddt_data = torch.zeros(batch, length)

    for batch_idx, (orig_mask_batch, lddt_aa_batch) in enumerate(zip(all_atom_mask_orig, lddt_data)):
        batch_sum=0
        for idx, res in enumerate(orig_mask_batch):
            new_vals = int(res.sum())
            #print(batch_sum, batch_sum + new_vals)
            avg = lddt_aa_batch[batch_sum: int(batch_sum + new_vals)].float().mean()
            avg = torch.nan_to_num(avg)
            batch_sum = batch_sum + new_vals
            residue_averaged_lddt_data[batch_idx,idx] = avg
            
    return residue_averaged_lddt_data
...
def lddt_loss(
    ...
    **kwargs,
    ...
    if ca_lddt:
        ...
        all_atom_mask_orig = all_atom_mask
        residue_level_mask = (all_atom_mask.sum(axis=2) > 0).float()
        all_atom_pred_pos, all_atom_positions, all_atom_mask = premask_aa(all_atom_pred_pos, 
                                                                        all_atom_positions, 
                                                                        all_atom_mask)
        n = int(all_atom_mask.sum(axis=1).max())
    ...
    if not ca_lddt:
        score = aggregate_to_residue_level_lddt(all_atom_mask_orig, score)
        score = score.to(logits.device)
    ...
    ###Weight by true LDDT values##### 
    map_to_real_lddt = lambda x: weight_vec[x]  #Fuction to get the weight value for any bin index
    applied_weights = map_to_real_lddt(lddt_ca_one_hot.argmax(dim=2))   #B*N of what to multiply loss element-wise by
    errors = errors * applied_weights  #Multiply them elementwise
    ##################################
    ...
    if ca_lddt:
        ...
        all_atom_mask = residue_level_mask.squeeze(-1)
        loss = torch.sum(errors * residue_level_mask, dim=-1) / (
            eps + torch.sum(residue_level_mask, dim=-1)
        )
...
def model_lddt_error(
    ...
    **kwargs,
    ...
    plddt_values = compute_plddt(logits) / 100.
    # plddt_values = logits.argmax(dim=2).float() #Get which value is highest predicted (0-49)
    # plddt_values = (0.02 * plddt_values) + 0.02 #Convert it to 0-1 lddt (bin index * 0.02)

    lddt_values = score
    # lddt_values = compute_plddt(lddt_ca_one_hot.float()) / 100.
    # lddt_values = lddt_ca_one_hot.argmax(dim=2).float() #Get which value is highest predicted (0-49)
    # lddt_values = (0.02 * lddt_values) + 0.02 #Convert it to 0-1 lddt (bin index * 0.02)

    modeled_residues = all_atom_mask[..., -1] #BXLX1 --> BXL - List which residues have experimentally resolved residues
    residue_errors = abs(plddt_values[:1] - lddt_values[:1]) #BXL (all resdiues have a value)
    masked_residue_errors = residue_errors * modeled_residues #BXL (zero out all non-experimentally resolved residues)
    model_errors = masked_residue_errors.sum(-1) / modeled_residues.sum(-1) # B vector (divide total error in each model by total number of resolved residues)
    #average_model_error = model_errors.mean() #Average over batch
